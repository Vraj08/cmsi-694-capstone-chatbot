...
from ..config import DAY_END, DAY_START
...
def matches_exact_oa_label(cell_value: str, canon_roster_name: str) -> bool:
    """True only if cell is exactly 'OA: <Roster Name>' (spacing tolerant)."""
...
def parse_time_str(t: str) -> time:
    ...
    if t in {"24:00", "24", "midnight", "12am", "12:00am"}:
        return time(0, 0)
    try:
...
def infer_range_am_pm(s_raw: str, e_raw: str, s: time, e: time) -> tuple[time, time]:
    ...
    if not s_mark and (e_mark == "pm" or e.hour >= 12):
        if 1 <= s.hour <= 11:
            s = time((s.hour % 12) + 12, s.minute)
    elif not s_mark and (e_mark == "am" or e.hour < 12):
        if s.hour == 12:
            s = time(0, s.minute)
    if not e_mark and s_mark in {"am", "pm"}:
        if s_mark == "pm" and e.hour < 12:
            e = time((e.hour % 12) + 12, e.minute)
        elif s_mark == "am" and e.hour == 12:
            e = time(0, e.minute)
    return s, e
...
def time_slots(start: time, end: time) -> list[time]:
    ...
    assert_30min_grid(start)
    assert_30min_grid(end)
    within_guardrails(start)
    if end != time(0, 0):
        within_guardrails(end)

    today = date.today()
    cur = datetime.combine(today, start)
    stop = (
        datetime.combine(today, time(23, 59)) + timedelta(minutes=1)
        if end == time(0, 0)
        else datetime.combine(today, end)
    )
    if stop <= cur:
        raise ValueError("End time must be after start time.")
...
def fmt_time(t: time | datetime) -> str:
